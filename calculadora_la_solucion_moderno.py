...
# Tema personalizado (para config.toml en .streamlit/)
st.set_page_config(page_title="Calculadora La Solución", layout="centered")

# Estilo de encabezado con HTML
st.markdown("""
...
if st.button("🔍 Resolver"):
    try:
        ...
        if operacion == "Evaluar":
            ...
            resultado = derivada
            pasos = f"Aplicamos la derivada a {funcion}:
Resultado: {derivada}"

        elif operacion == "Integral":
            integral = sp.integrate(funcion, x)
            resultado = integral
            pasos = f"Se calcula la integral de {funcion}:
